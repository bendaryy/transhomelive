                    ...
                    <table id="example2" class="table table-striped table-bordered text-center">
                        ...
                        <tbody>
                            ...
                            @foreach ($allInvoices as $index => $invoice)
                                ...
                                <tr>
                                    <td>{{ $invoice['source']['internalId'] }}</td>
                                    <td>{{ $invoice['source']['recipientName'] }}</td>
                                    <td>{{ $invoice['source']['submitterName'] }}</td>
                                    @if ($invoice['source']['submitterId'] == auth()->user()->details->company_id)
                                        <th style="background-color: green;color:white;opacity:0.8">فاتورة تم ارسالها</th>
                                        @else
                                        <th style="background-color: red;color:white;opacity:0.8">فاتورة تم استقبالها</th>
                                    @endif
                                    <td>{{ $invoice['source']['documentStatusAR'] }}</td>
                                    <td>{{ $invoice['source']['totalInvoiceAmount'] }}</td>
                                    <td>{{ Carbon\Carbon::parse($invoice['source']['submissionDate'])->format('d-m-Y') }}
                                    </td>
                                    <td>{{ $invoice['source']['uuid'] }}</td>
                                    <td><a href="https://invoicing.eta.gov.eg/print/documents/{{ $invoice['source']['uuid'] }}/share/{{ $invoice['source']['longId'] }} "
                                            target="_blank" class="btn btn-success">@lang('site.viewinportal')</a>
                                    </td>
                                    <td><a href="{{ route('pdf', $invoice['source']['uuid']) }}" class="btn btn-info"
                    ...
                    <nav aria-label="Page navigation example">
                        ...
                        <ul class="pagination">
                            <li class="page-item"><a class="page-link" {{ $id == 1 ? 'style=display:none' : '' }}
                                    href="{{ route('allinvoices', $id - 1) }}">السابق</a></li>
                            @for ($i = 1; $i <= $allMeta['totalPages']; $i++)
                                <li class="page-item"><a class="page-link"
                                        {{ $i == $id ? 'style=background-color:#CCC' : '' }}
                                        href="{{ route('allinvoices', $i) }}">{{ $i }}</a></li>
                            @endfor
                            <li class="page-item"><a class="page-link"
                                    {{ $id == $allMeta['totalPages'] ? 'style=display:none' : '' }}
                                    href="{{ route('allinvoices', $id + 1) }}">التالى</a></li>
